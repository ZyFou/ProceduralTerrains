...
export const SURFACE_UPLOAD_BYTES_PER_TASK = 512 * 1024;
export const SURFACE_UPLOAD_MS_PER_TASK = 2;
...
export async function uploadSurfaceTextures(renderer, atlas, {
  ...
  msPerTask = SURFACE_UPLOAD_MS_PER_TASK,
  onProgress,
} = {}) {
  ...
  try {
    // Include previously submitted rendering in the first fence. Allocation and
    // transfer remain on the render worker, but never accumulate unbounded GPU work.
    pacer.submitted();
    await waitForGpu();
    ...
    while (completed < jobs.length) {
      ...
      while (completed < jobs.length) {
        ...
        completed++;
        if (performance.now() - taskStarted >= msPerTask) break;
      }
      pacer.submitted();
      await waitForGpu();
