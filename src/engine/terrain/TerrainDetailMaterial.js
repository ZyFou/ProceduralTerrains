...
uniform highp sampler2DArray uDetailPageArray;
uniform vec3 uDetailPageCoords[16];
uniform float uDetailPageBasis;
uniform float uDetailGlobalBlend;

vec2 detailPagePosition(vec3 p) {
  if (uDetailPageBasis > 1.5) return p.xy;
  if (uDetailPageBasis > 0.5) return p.yz;
  return p.xz;
}

float detailPageSlot(vec2 tile) {
  return mod(tile.x, 4.0) + mod(tile.y, 4.0) * 4.0;
}

float detailPageAvailable(vec3 p) {
  vec2 tile = floor(detailPagePosition(p) / 128.0);
  int slot = int(detailPageSlot(tile));
  vec3 page = uDetailPageCoords[slot];
  return (1.0 - step(0.1, length(page.xy - tile))) * page.z;
}

vec4 detailPageSample(vec3 p) {
  vec2 coord = detailPagePosition(p);
  vec2 tile = floor(coord / 128.0);
  int slot = int(detailPageSlot(tile));
  vec3 page = uDetailPageCoords[slot];
...
float terrainDetailEnabled() {
  return step(0.5, uTerrainDetailQuality) * uDetailGlobalBlend;
}
...
float terrainDetailFadeAt(vec3 worldPos) {
  ...
  return fade * terrainDetailEnabled() * detailPageAvailable(worldPos)
    * clamp(uTerrainDetailOpacity, 0.0, 1.0);
}
...
float terrainDetailNoise(vec3 worldPos, vec3 n, float scale) {
  return detailPageSample(worldPos).r;
}
...
float terrainDetailRelief(vec3 worldPos, vec3 n, float scale) {
  vec4 page = detailPageSample(worldPos);
  float fine = page.r;
...
TerrainDetailResult applyTerrainDetailLayer(
  ...
  if (fade <= 0.001) {
    ...
    outD.detail = 0.5;
    outD.fade = 0.0;
    outD.rockMask = tc.rockBlend;
    outD.shoreMask = terrainShoreMask(hRel);
    return outD;
  }
